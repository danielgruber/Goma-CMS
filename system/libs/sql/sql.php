		...
		public static function extractToWhere($where, $includeWhere = true, $DBFields = array()) {
			...
			if(is_array($where) && count($where) > 0) {
				...
				foreach($where as $field => $value) {
					...
					if(is_array($value) && count($value) == 2 && isset($value[1], $value[0]) && ($value[0] == "LIKE" || $value[0] == ">" || $value[0] == "<")) {
						if($value[0] == "LIKE") {
							$sql .= ' '.convert::raw2sql($field).' '.(defined("SQL_LIKE") ? SQL_LIKE : "LIKE").' "'.convert::raw2sql($value[1]).'"';
						} else if($value[0] == ">") {
							$sql .= ' '.convert::raw2sql($field).' > "'.convert::raw2sql($value[1]).'"';
						} else if($value[0] == "<") {
							$sql .= ' '.convert::raw2sql($field).' < "'.convert::raw2sql($value[1]).'"';
						}
...
interface SQLDriver
{
		...
		*/
		 ...
		 * table-functions V2
		*/
		public function showTableDetails($table, $track = true);
		public function requireTable($table, $fields, $indexes, $defaults);
