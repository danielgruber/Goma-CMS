initRadioButton = function(field, divid) {
    var div = $("#" + divid);
    var isSelect = div.find("> .select_wrapper > select").length > 0;

    field.getValue = function() {
        if(isSelect) {
            return $("#" + divid).find("select option:selected").attr("value");
        }
    ...
    field.setValue = function(value) {
        if(isSelect) {
            var option = div.find("> .select_wrapper select option[value=" + value + "]");
            if(option.length == 1 && !option.prop("disabled")) {
                div.find("> .select_wrapper select option:selected").prop("selected", false);
                option.prop("selected", true);
                div.find("> .select_wrapper select").change();
            }
        } else {
    ...
    field.getPossibleValuesAsync = function() {
        ...
        setTimeout(function(){
            var values = [];
            if(isSelect) {
                div.find("> .select_wrapper select option").each(function () {
