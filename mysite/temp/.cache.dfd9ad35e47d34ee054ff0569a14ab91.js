...
{$("body").css("cursor","wait");$.ajax({cache:true,noRequestTrack:true,url:BASE_SCRIPT+"gloader/"+component+".js",dataType:"script",error:function(jqXHR,textStatus,errorThrown){alert(textStatus);alert(errorThrown);},async:false});$("body").css("cursor","auto");gloader.loaded[component]=true;if(fn!=null)
fn();}},loaded:[]};array_shuffle=function(array){var tmp,rand;for(var i=0;i<array.length;i++){rand=Math.floor(Math.random()*array.length);tmp=array[i];array[i]=array[rand];array[rand]=tmp;}
...
w.eval_script=function(html,ajaxreq,object){LoadAjaxResources(ajaxreq);if(typeof profiler!="undefined")profiler.mark("eval_script");var content_type=ajaxreq.getResponseHeader("content-type");if(content_type=="text/javascript"){if(typeof object!="undefined"){var method;if(window.execScript)
window.execScript('method = '+'function('+html+')','');else
var method=eval_global('(function(){'+html+'});');method.call(object);}else{if(window.execScript)
window.execScript(html,'');else
eval(html);}}else if(content_type=="text/x-json"){var object=eval_global("("+html+")");var _class=object["class"];var i;for(i in object["areas"]){$("#"+_class+"_"+i+"").html(object["areas"][i]);}}else{gloader.load("orangebox");var id=randomString(5);if(html_regexp.test(html)){self[id+"_html"]=html;$("body").append('<div id="'+id+'_div" style="display: none;width: 800px;hieght: 300px;"><iframe src="javascript:document.write(top.'+id+'_html);" height="500" width="100%" name="'+id+'" frameborder="0" id="'+id+'"></iframe></div>');$("body").append('<a style="display: none;" href="#'+id+'_div" rel="orangebox" id="'+id+'_link"></a>');$("#"+id+"_link").click();}else{$("body").append('<div id="'+id+'_div" style="display: none;">'+html+'</div>');$("body").append('<a style="display: none;" href="#'+id+'_div" rel="orangebox" id="'+id+'_link"></a>');$("#"+id+"_link").click();}}
...
function eval_global(codetoeval){if(window.execScript)
