...
<?php $data->convertDefault = false; if($caller->permission("BOXES")&&! $caller->adminAsUser()) { $data->convertDefault = null; ?>
	...
	<script type="text/javascript">
		// <![CDATA[
			(function($){
				$(function(){
					$("#boxes_new_<?php echo $data["pageid"]; ?> .box_new").resizable({
						autoHide: true,
						handles: 'e',
						minWidth: 100,
						grid: [10, 10],
						stop: function(event, ui) {
							$.ajax({
								url: root_path + "boxes_new/<?php echo $data["pageid"]; ?>/saveBoxWidth/" + ui.element.attr("id").replace("box_new_", ""),
								type: "post",
					...
					$("#boxes_new_<?php echo $data["pageid"]; ?>").sortable({
						opacity: 0.6,
						handle: '.adminhead',
						helper: 'clone',
						...
						update: function(event, ui) {
							...
							// save order
							$.ajax({
								url: root_path + "boxes_new/<?php echo $data["pageid"]; ?>/saveBoxOrder",
								data: data,
								type: "post",
								dataType: "html"
							});
					...
					$("#boxes_new_<?php echo $data["pageid"]; ?> > .box_new .adminhead").css("cursor", "move");
				});
			})(jQuery);
		// ]]>
	</script>
...
<div class="boxes_new" id="boxes_new_<?php echo $data["pageid"]; ?>">
...
?>
			...
			<?php $data->convertDefault = false; if($caller->permission("BOXES")&&! $caller->adminAsUser()&&($data->doObject("box")->doObject("title") && $data->doObject("box")->doObject("title")->bool())) { $data->convertDefault = null; ?>
			...
			<?php $data->convertDefault = false; } else if($caller->permission("BOXES")&&! $caller->adminAsUser()) {  $data->convertDefault = null; ?>
			...
			<?php }   $data->convertDefault = null; ?>
				<?php $data->convertDefault = false; if($caller->permission("BOXES")&&! $caller->adminAsUser()) { $data->convertDefault = null; ?>
					<div class="adminhead">
						<div class="actions">
							<a href="<?php echo defined("BASE_SCRIPT") ? constant("BASE_SCRIPT") : null; ?>boxes_new/<?php echo $data["pageid"]; ?>/add?insertafter=<?php echo $data["box"]["sort"]; ?>&redirect=<?php echo (isset($data["_SERVER_REDIRECT"]) ? $data->doObject("_SERVER_REDIRECT")->URL() : ""); ?>" title="<?php echo lang("new_box", "new_box"); ?>" rel="dropdownDialog">
								<img src="images/16x16/add.png" alt="<?php echo lang("new_box", "new_box"); ?>" />
							</a>
							
							<a class="noAutoHide" href="<?php echo defined("BASE_SCRIPT") ? constant("BASE_SCRIPT") : null; ?>boxes_new/<?php echo $data["pageid"]; ?>/edit/<?php echo $data["box"]["id"]; ?>?redirect=<?php echo (isset($data["_SERVER_REDIRECT"]) ? $data->doObject("_SERVER_REDIRECT")->URL() : ""); ?>" title="<?php echo lang("edit_box", "edit_box"); ?>" rel="dropdownDialog">
								<img src="images/16x16/edit.png" alt="<?php echo lang("edit_box", "edit_box"); ?>" />
							</a>
							
							<a class="" href="<?php echo defined("BASE_SCRIPT") ? constant("BASE_SCRIPT") : null; ?>boxes_new/<?php echo $data["pageid"]; ?>/delete/<?php echo $data["box"]["id"]; ?>?redirect=<?php echo (isset($data["_SERVER_REDIRECT"]) ? $data->doObject("_SERVER_REDIRECT")->URL() : ""); ?>" title="<?php echo lang("del_box", "del_box"); ?>" rel="dropdownDialog">
...
?>
	...
	<?php } else { ?>
		<?php $data->convertDefault = false; if($caller->permission("BOXES")&&! $caller->adminAsUser()) { $data->convertDefault = null; ?>
			<div class="no_box">
				<a rel="dropdownDialog" href="<?php echo defined("BASE_SCRIPT") ? constant("BASE_SCRIPT") : null; ?>boxes_new/<?php echo $data["pageid"]; ?>/add?insertafter=1"><?php echo lang("new_box", "new_box"); ?></a>
			</div>
		<?php }   $data->convertDefault = null; ?>
	<?php }   $data->convertDefault = null; ?>
</div>